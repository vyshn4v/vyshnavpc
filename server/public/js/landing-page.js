/* =====================================================================
   ...
   Skills data is injected by the server via window.__DATA__ so this
   file stays generic and reusable.
   ===================================================================== */

/* ── SKILLS PILLS ──────────────────────────────────────────────────── */
(function () {
  const categories = (window.__DATA__ && window.__DATA__.skillCategories) || [];
  categories.forEach(function (cat) {
    const el = document.getElementById("sk-" + cat.id);
    if (!el) return;
    const mk = () =>
      cat.pills
        .map(
        ...
        .join("");
    el.innerHTML = mk() + mk(); // duplicate for infinite scroll
  });
})();
...
(function () {
  ...
  if (!cvs || window.matchMedia("(prefers-reduced-motion:reduce)").matches)
    return;
...
function sendMsg() {
  ...
  const to =
    (window.__DATA__ && window.__DATA__.email) || "vyshnavpcnaravoor@gmail.com";
